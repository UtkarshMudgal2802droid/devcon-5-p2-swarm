---
name: loops-road-to-devcon-v
description: >-
  Build for the Road To Devcon - V on Loops House: ideate with the AI
...
event:
  ...
  submissionDeadline: "Sep 21, 2026, 1:11 AM (Asia/Calcutta)"
  registrationDeadline: "Sep 19, 2026, 11:11 PM (Asia/Calcutta)"
  description: null
problems[3]:
  - title: "Eight hundred winters, one lapsed invoice"
    slug: archive-that-outlives-its-host
    brief: "In a stone vault above the Spiti valley, birch-bark folios have survived eight hundred winters. Cold, dry, dark, and largely left alone, the conditions turned out to be close to ideal, and nobody designed them. Tsering spent two winters photographing them. Eleven thousand images: Bhojpatra scrolls, palm-leaf folios, a medical compendium with margin notes in three hands. Page by page, in gloves, by window light. The photographs live in a cloud account a visiting researcher opened in 2019 under a grant. The grant ended. The researcher moved institutions. Last March the account was suspended for non-payment, and it took five weeks and four emails to someone who no longer works there to get it b…"
    successLooksLike: "Tsering can hand someone a single address, delete the app, and that person still gets every folio back."
    suggestedStack[5]: Swarm Desktop,bee-js,swarm-cli,Node.js,TypeScript
    judgingCriteria[1]{name,weightPct}:
      "Problem interpretation, product judgment & code craft",20
  - title: Take your records with you
    slug: take-your-records-with-you
    brief: "The Deccan Birders have kept sighting records since 1998. Species, date, place, who saw it, sometimes a photo. They have been through three apps. A forum that closed. A Facebook group that ate the photos and cannot be searched by date. A birding app that was acquired and shut down, and gave them a CSV export where the location column was a string like \"near the usual spot\". Meera, who maintains the list now, has one requirement before she moves the group anywhere again: whatever writes the records must not be the only thing that can read them. **What you'll learn** Uploading on behalf of a user who has no tokens. What has to travel with a record for it to be readable by software that has nev…"
    successLooksLike: "Meera files a sighting in one app and opens it in a completely different one, and nobody had to export anything."
    suggestedStack[5]: Swarm ID,bee-js,React,TypeScript,Swarm Desktop
    judgingCriteria[1]{name,weightPct}:
      "Problem interpretation, product judgment & code craft",20
  - title: The succession nobody wrote down
    slug: steward-succession
    brief: "Seven monastery libraries across Ladakh and Spiti contribute to one shared catalogue: what each collection holds, the condition of every folio, which are damaged, which are missing, which have been photographed and which have not. Nowhere else does the whole picture exist. Ngawang Dorje has maintained it for nine years. He renews the storage. He holds the key that publishes updates. The other six send him corrections by email and WhatsApp and he enters them himself. He is seventy-one. These are institutions with centuries of practice at handing responsibility from one holder to the next, an unbroken line of custodians is the entire reason the manuscripts still exist. Their catalogue has none…"
    successLooksLike: "Ngawang Dorje stops answering email and the catalogue is still reachable, still paid for, and still correctable by the other six."
    suggestedStack[5]: bee-js,Swarm Desktop,TypeScript,Safe (optional),Gnosis Chain (optional)
    judgingCriteria[1]{name,weightPct}:
      "Problem interpretation, product judgment & code craft",20
```
...
```sh
loops credits --event road-to-devcon-v
```
...
loops knowledge query --event road-to-devcon-v --problem archive-that-outlives-its-host -q "<your question about Eight hundred winters, one lapsed invoice>"

# Take your records with you
loops knowledge query --event road-to-devcon-v --problem take-your-records-with-you -q "<your question about Take your records with you>"

# The succession nobody wrote down
loops knowledge query --event road-to-devcon-v --problem steward-succession -q "<your question about The succession nobody wrote down>"
```
...
loops project create --event road-to-devcon-v --repoUrl <url>
...
```sh
# Eight hundred winters, one lapsed invoice
loops evaluate --event road-to-devcon-v --problem archive-that-outlives-its-host

# Take your records with you
loops evaluate --event road-to-devcon-v --problem take-your-records-with-you

# The succession nobody wrote down
loops evaluate --event road-to-devcon-v --problem steward-succession
```
